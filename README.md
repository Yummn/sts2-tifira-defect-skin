# 蒂菲拉-故障机器人皮肤增强版 (TifiraDefectSkin) v1.1.2

Slay the Spire 2 故障机器人皮肤替换/增强 MOD。下载 Releases 里的 zip，解压后把 `TifiraDefectSkin` 文件夹复制到 `mods/`。

## 功能
- 将故障机器人替换为《卡厄斯梦境》的蒂菲拉。
- 选角界面替换为蒂菲拉主题形象，并提供“切换背景”按钮。
- 战斗 body 替换为蒂菲拉 Spine，入场后进入待机。
- 长按/拖出手牌时显示左侧 Battle Ready 立绘演出，打出攻击/技能时切换对应 cut-in；v1.1.1 起入场/出场带淡入淡出。
...
- 本增强版只在原资源基础上补增强触发逻辑、manifest 和打包；角色图片、Spine、音频等素材仍来自原包。
...
- 普通攻击：`attack`；多段/群体攻击：`attack2`；格挡/防御/支援：`cast2`；充能球充能：`cast3`；充能球激发、稀有牌、2 费以上或 UG/UX/大招类：`cast4`；受击：`hurt`；胜利：`victory_ready -> victory`。
...
- `Scripts/*.cs`：原 PCK 内脚本留存内容，当前增强逻辑由外部 DLL `TifiraDefectSkinCode/MainFile.cs` 实现。
...
- v1.1.2：修复 v1.1.1 包内 `TifiraDefectSkin.json` 描述字段缺少结束引号导致 manifest 不是合法 JSON 的问题；DLL 仅同步日志版本号，功能保持 v1.1.1。